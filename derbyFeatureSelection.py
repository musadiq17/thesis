...
from derbyFeatureExtraction import EndFeatureDict
...
port = PorterStemmer()
detoken = ''
unwantedTerms = ['.java' ,'.apache' ,'.security' ,'.derby' ,'.derbytesting' ,'.accesscontrolexception' ,'&#91' ,'.functiontests'  ,'java.lang' ,'.impl' ,'java.io.'
,':derby' ,'.sql' ,'.properties' ,'.tests' ,'.securitymanager.checkpermission('  ,'.accesscontroller.checkpermission(accesscontroller', '.jar'  ]
...
with open('derby.csv', 'r') as csv_file:
...
# -----------Write CSV File---------------------------
with open('derbyFeatures.csv', 'w', newline='') as f:
